...
static void ForceApplyHeaderTexts(HWND hList) {
    ...
    for (int ci = 0; ci < 4; ++ci) {
        ...
        // Ensure text is treated as a string by the header control
        hi.fmt |= HDF_STRING;
        hi.pszText = (LPWSTR)g_colHeaders[ci].c_str();
        SendMessageW(hHeader, HDM_SETITEMW, ci, (LPARAM)&hi);
    }
    InvalidateRect(hHeader, NULL, TRUE);
    // Force immediate redraw of header and listview
    RedrawWindow(hHeader, NULL, NULL, RDW_INVALIDATE | RDW_ERASE | RDW_UPDATENOW);
    RedrawWindow(hList, NULL, NULL, RDW_INVALIDATE | RDW_ERASE | RDW_UPDATENOW | RDW_FRAME);
...
// message to reapply header/text/format after window is shown and layout settled
#define WM_REAPPLY_HEADERS (WM_APP + 10)
// Timer used to defer reapplying header texts (helps avoid race with header creation)
#define REAPPLY_HEADERS_TIMER_ID 0xC0FF
...
static bool g_reapply_msg_shown = false;
...
static bool g_header_subclassed = false;
