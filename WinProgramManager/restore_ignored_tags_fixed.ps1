...
foreach ($pkg in $packageList) {
    ...
    # Query winget and count tags (with retry logic)
    ...
    while ($retryCount -lt $maxRetries -and -not $success) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            if ($retryCount -lt $maxRetries) {
                Write-Log "  WARNING: Winget query failed, retrying ($retryCount/$maxRetries)..." "WARNING"
    ...
    if (-not $success) {
        Write-Log "  ERROR: Failed to query winget after $maxRetries attempts" "ERROR"
        $errors++
        continue
    }
